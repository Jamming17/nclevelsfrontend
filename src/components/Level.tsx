...
function Level({level, getData} : LevelProps) {
    ...
    return (
        <div className="flex flex-row rounded-xl border-2 border-gray-500 mx-10 my-3 w-[600px] md:w-[800px] lg:w-[1000px] xl:w-[1200px] h-[200px] ">
            {/* Level Thumbnail */}
            <div>
                <img className="rounded-l-xl md:w-auto max-w-[350px] w-[230px] h-full object-cover mr-2" src={`https://levelthumbs.prevter.me/thumbnail/${currentLevel.id}/small`}></img>
            </div>

            {/* Difficulty Face + Stars/Moons Awarded + Coins */}
            <div className="flex flex-col items-center max-w-[80px] py-1">
                <img src={`/assets/difficulties/${currentLevel.difficulty.toLowerCase().replace(/\s+/g,"")}-${currentLevel.rating.toLocaleLowerCase().replace(/\s+/g,"")}.png`} alt={`GD Difficulty face: ${currentLevel.difficulty} with rating ${currentLevel.rating}`}/>
                ...
                <div className="flex flex-row items-center mb-1">
                    ...
                    {currentLevel.length == "Plat." ?
                        <img className="max-h-[25px]" src="/assets/stats/moon.png" alt="moon" />
                        :
            ...
            {/*<button onClick={reloadLevelData}>h</button>*/}
